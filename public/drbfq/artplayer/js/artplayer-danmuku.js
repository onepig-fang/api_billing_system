    ...
    "8Gtnh": [function (t, e, n) {
        ...
        class s {
            ...
            /* constructor(e, n) {
                 ...
                 // 添加对 seeked 事件的监听
                 e.on("video:seeked", this.onSeeked.bind(this));
             }*/
            constructor(e, n) {
                const {constructor: i, template: r} = e;
                if (this.utils = i.utils, this.validator = i.validator, this.$danmuku = r.$danmuku, this.$player = r.$player, this.art = e, this.queue = [], this.option = {}, this.$refs = [], this.isStop = !1, this.isHide = !1, this.timer = null, this.config(n), this.option.useWorker) try {
                ...
                this.onSeeked = this.onSeeked.bind(this); // 确保 onSeeked 方法被绑定
                e.on("video:play", this.start);
                e.on("video:playing", this.start);
                e.on("video:pause", this.stop);
                e.on("video:waiting", this.stop);
                e.on("fullscreen", this.reset);
                e.on("fullscreenWeb", this.reset);
                e.on("destroy", this.destroy);
                this.load();

                // 添加对 seeked 事件的监听
                e.on("video:seeked", this.onSeeked);
            }

            onSeeked() {
                // 当视频进度被改变时，重置弹幕状态或者重新加载弹幕
                this.reset();
            }
            ...
            get isRotate() {
                return this.art.plugins && this.art.plugins.autoOrientation && this.art.plugins.autoOrientation.state;
            }
            ...
            parseDanmukuXML(xmlText) {
                try {
                    const parser = new DOMParser();
                    const xmlDoc = parser.parseFromString(xmlText, "text/xml");
                    const danmukuNodes = xmlDoc.querySelectorAll("d");
                    const danmukuData = [];

                    danmukuNodes.forEach(node => {
                        const pAttr = node.getAttribute("p");
                        if (pAttr) {
                            const [time, mode, fontSize, color, ...rest] = pAttr.split(",");
                            danmukuData.push([
                                time,
                                mode === "1" ? "top" : mode === "2" ? "bottom" : "right",
                                color,
                                "",
                                node.textContent || ""
                            ]);
                    ...
                    console.warn("Failed to parse XML danmuku:", e);
                    return [];
                }
            }
            ...
            async load() {
                try {
                    let danmukuData = [];
                    const rawDanmuku = this.option.danmuku;

                    // 处理API请求获取弹幕数据
                    if (typeof rawDanmuku === 'string' && rawDanmuku.startsWith('http')) {
                        try {
                            const response = await fetch(rawDanmuku);
                            if (!response.ok) {
                                throw new Error(`Failed to fetch danmuku data: ${response.status}`);
                            }
                            const data = await response.json();
                            //console.log("API返回原始数据结构:", data); // 关键：打印完整数据结构

                            // 动态检测数据结构
                            if (data.danmuku && Array.isArray(data.danmuku)) {
                                danmukuData = data.danmuku; // 适配含danmuku字段的格式
                            } else if (data.data && Array.isArray(data.data)) {
                                danmukuData = data.data; // 适配含data字段的格式
                            } else if (Array.isArray(data)) {
                                danmukuData = data; // 直接是数组的格式
                            } else {
                                throw new Error("Unsupported danmuku data structure");
                            }
                        } catch (err) {
                            try {
                                const text = await (await fetch(rawDanmuku)).text();
                                danmukuData = this.parseDanmukuXML(text);
                            } catch (xmlErr) {
                                throw new Error(`Failed to fetch and parse danmuku: ${err.message}, ${xmlErr.message}`);
                            }
                        }
                    } else if (rawDanmuku !== undefined) {
                        danmukuData = Array.isArray(rawDanmuku) ? rawDanmuku : [rawDanmuku];
                    }

                    // 增强型数据解析（适配多种格式）
                    const parsedDanmuku = [];
                    // console.log("danmukuData:", danmukuData);
                    if (Array.isArray(danmukuData)) {
                        danmukuData.forEach((item, index) => {
                            let danmukuObj = {};

                            // 尝试解析对象格式（多种字段名适配）
                            if (typeof item === 'object' && !Array.isArray(item)) {
                                danmukuObj = {
                                    text: item.text || item.content || item.danmuku_text || "", // 适配不同文本字段名
                                    time: parseFloat(item.time || item.start_time || item.danmuku_time || 0), // 适配不同时间字段名
                                    mode: parseInt(item.mode || item.danmuku_mode || 0), // 适配不同模式字段名
                                    color: item.color || item.danmuku_color || "#FFFFFF", // 适配不同颜色字段名
                                    fontSize: item.fontSize || item.font_size || "24px" // 适配不同字体大小字段名
                                };
                            }
                            // 尝试解析数组格式（兼容不同长度）
                            else if (Array.isArray(item)) {
                                // 适配 [time, mode, color, ..., text] 格式
                                const time = item[0] !== undefined ? parseFloat(item[0]) : 0;
                                const modeStr = item[1] || "";
                                const color = item[2] || "#FFFFFF";
                                const text = item[4] || ""; // 弹幕文本位置

                                danmukuObj = {
                                    text,
                                    time,
                                    mode: modeStr === 'top' ? 1 : modeStr === 'bottom' ? 2 : 0,
                                    color,
                                    fontSize: item[6] || item[7] || "24px" // 尝试获取字体大小
                                };
                            }

                            // 确保有有效文本
                            if (danmukuObj.text.trim() !== "") {
                                parsedDanmuku.push(danmukuObj);
                            } else {
                                console.log(`跳过空文本弹幕，原始数据:`, item);
                            }
                        });
                    } else {
                        throw new Error("Danmuku data must be an array");
                    }

                    //console.log("解析后的弹幕数据:", parsedDanmuku);
                    this.utils.errorHandle(Array.isArray(parsedDanmuku), "Danmuku data must be an array");

                    this.art.emit("artplayerPluginDanmuku:loaded", parsedDanmuku);
                    this.queue = [];
                    this.$danmuku.innerText = "";

                    parsedDanmuku.forEach(danmuku => {
                        this.emit(danmuku);
                        // console.log("发射弹幕:", danmuku.text, "时间:", danmuku.time);
                    });

                    return this;
                } catch (err) {
                    this.art.emit("artplayerPluginDanmuku:error", err);
                    console.error("弹幕加载错误:", err);
                    throw err;
                }
            }
            ...
            update() {
                this.timer = window.requestAnimationFrame(async () => {
                    if (this.art.playing && !this.isHide) {
                        ...
                        // 筛选准备发射的弹幕
                        const readyDanmuku = this.queue.filter(danmuku => {
                            return danmuku.$state === "ready" ||
                                (danmuku.$state === "wait" &&
                                    currentTime + 0.1 >= danmuku.time &&
                                    danmuku.time >= currentTime - 0.1);
                        });

                        const {clientWidth, clientHeight} = this.$player;
                        for (let i = 0; i < readyDanmuku.length; i++) {
                            const danmuku = readyDanmuku[i];
                            danmuku.$ref = this.getRef();
                            danmuku.$ref.innerText = danmuku.text;
                            this.$danmuku.appendChild(danmuku.$ref);

                            // 设置弹幕样式
                            danmuku.$ref.style.left = `${clientWidth}px`;
                            danmuku.$ref.style.opacity = this.option.opacity;
                            danmuku.$ref.style.fontSize = `${this.getFontSize(this.option.fontSize)}px`;
                            danmuku.$ref.style.color = danmuku.color;
                            danmuku.$ref.style.border = danmuku.border ? `1px solid ${danmuku.color}` : null;
                            danmuku.$ref.style.backgroundColor = danmuku.border ? "rgb(0 0 0 / 50%)" : null;

                            danmuku.$lastStartTime = Date.now();
                            danmuku.$restTime = this.option.synchronousPlayback && this.art.playbackRate
                                ? this.option.speed / Number(this.art.playbackRate)
                                : this.option.speed;

                            // 获取弹幕位置
                            const position = await this.postMessage({
                                target: {
                                    mode: danmuku.mode,
                                    height: danmuku.$ref.clientHeight,
                                    speed: (clientWidth + danmuku.$ref.clientWidth) / danmuku.$restTime
                                },
                                emits: this.getEmits(),
                                antiOverlap: this.option.antiOverlap,
                                clientWidth,
                            ...
                            if (danmuku.$ref && position.top !== undefined) {
                                danmuku.$state = "emit";
                                danmuku.$ref.style.visibility = "visible";

                                // 根据模式设置弹幕位置
                                if (danmuku.mode === 0) { // 滚动模式
                                    danmuku.$ref.style.top = `${position.top}px`;
                                    const startX = clientWidth + danmuku.$ref.clientWidth;
                                    danmuku.$ref.style.transform = `translateX(${-startX}px)`;
                                    danmuku.$ref.style.transition = `transform ${danmuku.$restTime}s linear 0s`;
                                } else if (danmuku.mode === 1) { // 顶部固定
                                    danmuku.$ref.style.left = "50%";
                                    danmuku.$ref.style.top = `${position.top}px`;
                                    danmuku.$ref.style.marginLeft = `-${danmuku.$ref.clientWidth / 2}px`;
                                }
                            } else {
                                danmuku.$state = "ready";
                                this.$refs.push(danmuku.$ref);
                                danmuku.$ref = null;
                            }
                        }
                    }
            ...
            emit(t) {
                const danmuku = {
                    text: t.text || "",
                    time: t.time !== undefined ? t.time : this.art.currentTime + 0.5,
                    mode: t.mode !== undefined ? t.mode : 0,
                    color: t.color || this.option.color,
                    ...t
                };

                // 暂时移除过滤条件，确保所有有效文本弹幕被添加
                if (danmuku.text.trim()) {
                    danmuku.time = this.utils.clamp(danmuku.time, 0, Infinity);
                    danmuku.mode = Math.max(0, Math.min(2, danmuku.mode || 0)); // 确保mode在0-2之间
                    ...
                    // console.log("添加到队列的弹幕:", danmuku.text, "时间:", danmuku.time);
                }
                return this;
            }
            ...
            t.once("video:loadedmetadata", (() => {
                x(), t.setting.add({
                    ...
                    selector: [{
                        ...
                        tooltip: "1/4",
                        selector: [
                        {default: !0, html: "1/4", margin: [10, "75%"]}, 
                        {html: "半屏", margin: [10, "50%"]}, 
                        {html: "3/4", margin: [10, "25%"]}, 
