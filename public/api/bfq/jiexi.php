...
$api = 'http://'.$_SERVER["HTTP_HOST"].'/api.php/?key='.$userKey.'&url='; // JSON 接口
//调用方法你的域名+jiexi.php?url=视频地址或者将：域名+jiexi.php?url=放进播放器后台josn接口
// IP 授权功能
$ipsq = false; // true 开启 IP 授权，false 关闭
...
define('MAX_DOWNLOAD_BYTES', 10 * 1024 * 1024);
...
if (isset($_GET['url'])) {
    ...
    if (!is_safe_url($targetUrl)) {
        echo json_encode(['code' => 400, 'msg' => 'URL不允许访问'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }
    ...
    if (curl_errno($ch)) {
        ...
        if (json_last_error() == JSON_ERROR_NONE && isset($decodedResponse['url'])) {
            ...
            if (!is_safe_url($parsedUrl)) {
                echo json_encode(['code' => 400, 'msg' => '解析结果URL不允许访问'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                exit;
            }
...
function download_file($url) {
    ...
    curl_setopt($ch, CURLOPT_WRITEFUNCTION, function ($ch, $chunk) use (&$bytes, &$content) {
        $bytes += strlen($chunk);
...
function is_safe_url($url) {
    ...
    if (!$records) {
        $records = [['ip' => gethostbyname($parsed['host'])]];
    }
