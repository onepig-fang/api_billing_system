layui.define(['table', 'laypage','jquery', 'element','laytpl'], function(exports) {
	...
	var defaultOption = {
		...
		request: {
			...
			, authorName: 'author' //时间名称，默认：time
			, versionName: 'version' //时间名称，默认：time
	...
	function getCheckedData(obj,elem) {
		...
		for (var i = 0; i < option.data.length; i++) {
			if (item.id == option.data[i].id) {
				...
				item[option.request.authorName] = option.data[i].author;
				item[option.request.versionName] = option.data[i].version;
				item[option.request.end_time] = option.data[i].end_time;
				...
				if (option.request.authorName != "author") {
					delete item.author;
				}
				if (option.request.versionName != "version") {
					delete item.version;
				}
				if (option.request.end_timeName != "end_time") {
	...
	function createCards(linenum, data, toolbar,image) {
		var content = "<div class='layui-row layui-col-space30 list-box'>";
	...
	function createCard(linenum, item, index, toolbar,image) {
		...
		{
		return '<div type=card id=card_' + item.id + ' class="layui-col-md' + line + ' ew-datagrid-item" data-index="' + index + '" data-number="1" > <div class="item">' + (!!item.image ? ' <div class="t"><div class="left" lay-event="details"><img class="project-list-item-cover img" src="' + item.image + '"></div>' : '') + '<div class="right"> <div class="title" lay-event="details">' + item.title + '</div> <div class="price free" lay-event="details">' + item.time + '</div><div class="install" lay-event="details">到期时间：' + item.end_time + '</div><div class="install" lay-event="details">' + item.remark + '</div><div class="install" lay-event="details">var:<span>' + item.version + '</span></div></div> </div>  <div class="b"> <span class="author"  >作者：<span>' + item.author + '</span></span><div class="y">' + (!!template ? ' <div class="ew-head-list">' + template+'</div>':'') +' </div> </div > </div > </div > ';
		}	
	...
	function initData(tempData, option) {
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			item.author = dataList[i][option.request.authorName];
			item.version = dataList[i][option.request.versionName];
			item.end_time = dataList[i][option.request.end_timeName];
			...
			if (option.request.authorName != "author") {
				delete item[option.request.authorName];
			}
			if (option.request.versionName != "version") {
				delete item[option.request.versionName];
			}
			if (option.request.end_timeName != "end_time") {
				delete item[option.request.end_timeName];
			}
			data.data.push(item);
		}
		return data;
	...
	var tt = {
		...
		getAllData: function (id) {
			...
			for (var i = 0; i < option.data.length; i++) {
				...
				item[option.request.authorName] = option.data[i].author;
				item[option.request.versionName] = option.data[i].version;
				item[option.request.end_timeName] = option.data[i].end_time;
				item = $.extend(JSON.parse(JSON.stringify(option.data[i])), item);
				if (option.request.idName != "id") {
					delete item.id;
				}
